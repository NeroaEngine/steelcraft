...
function LoginScreen({ config, status, vault, error }) {
  return <main className="os-login"><section className="login-card"><div className="logo-lockup"><img src="/brand/scb-logo.png" alt="Steel Craft" /><span>Steel Craft OS</span></div><h1>One job record. One Vault. One operating system.</h1><p>Sign in with Microsoft through the <b>Neroa Auth Spine</b>. Microsoft is the identity provider only; Neroa issues the Steel Craft tenant session.</p>{error ? <div className="os-alert danger">{error}</div> : null}<a className={`primary-login ${config?.microsoftConfigured ? '' : 'disabled'}`} href="/api/os/auth/microsoft/start">Continue with Microsoft</a>{!config?.microsoftConfigured ? <div className="os-alert">Auth is waiting for the Neroa Auth Spine registration. Configure NEROA_AUTH_SPINE_URL and NEROA_AUTH_CLIENT_ID. Do not put raw Microsoft client secrets in Steel Craft.</div> : null}<div className="login-meta"><span>Authority: {config?.authority || 'neroa_auth_spine'}</span><span>Schema: {status?.schema || 'steelcraft_os_v1'}</span><span>Vault: {vault?.canonicalVaultId || status?.vaultId || 'vault_steelcraft_001'}</span><span>Policy: {vault?.policyMode || 'fail_closed'}</span></div></section></main>;
}
