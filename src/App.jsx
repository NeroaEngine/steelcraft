...
function DeveloperRoom({ moduleMap, setModuleMap, enabledPortals, setEnabledPortals, portals }) {
  const [locked, setLocked] = useState(isDeveloperLocked());
  const selectedPack = getIndustryPack(moduleMap.industryPack);
  const selectedStatus = getIndustryPackWiringStatus(moduleMap.industryPack);
  ...
  const selectPack = (packId) => { const next = createTenantModuleMap({ tenantId: moduleMap.tenantId, tenantName: moduleMap.tenantName, industryPack: packId }); setModuleMap(next); saveTenantModuleMap(next); const nextEnabled = [...next.enabledCanonicalPortals, ...next.enabledIndustryPortals]; setEnabledPortals(nextEnabled); saveEnabledPortals(nextEnabled); };
...
function App() {
  ...
  if (path === '/developer') { if (user.role !== 'developer') { goTo('/portal/admin'); return null; } return <DeveloperShell brand={brand} user={user} signOut={signOut}><DeveloperRoom moduleMap={moduleMap} setModuleMap={setModuleMap} enabledPortals={enabledPortals} setEnabledPortals={setEnabledPortals} portals={portals} /></DeveloperShell>; }
