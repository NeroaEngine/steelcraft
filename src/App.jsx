...
function toAppUser(dbUser, moduleMap = loadTenantModuleMap()) {
  const customerPortalIds = allPortalIds(moduleMap);
  const role = dbUser?.role || 'admin';
  const rolePortals = { developer: ['developer', ...customerPortalIds], admin: customerPortalIds, accounting: ['accounting', 'contacts', 'employee'], employee: ['employee'], vendor: ['vendor'], customer: ['customer'] };
  return { id: String(dbUser?.id || role), dbId: dbUser?.id, role, name: dbUser?.name || dbUser?.full_name || 'User', email: dbUser?.email || '', language: dbUser?.language || 'en', portals: rolePortals[role] || rolePortals.employee };
}
...
function Shell({ brand, user, active, signOut, children, enabledPortals, portals, meta }) {
...
function DeveloperRoom({ moduleMap, setModuleMap, enabledPortals, setEnabledPortals, portals, meta }) {
  const [locked, setLocked] = useState(isDeveloperLocked());
  const selectedPack = getIndustryPack(moduleMap.industryPack);
  const canonicalRows = portals.filter(([id, , , , record]) => record?.scope === 'canonical');
  const industryRows = portals.filter(([id, , , , record]) => record?.scope === 'industry');
  ...
  const selectPack = (packId) => {
    const next = createTenantModuleMap({ tenantId: moduleMap.tenantId, tenantName: moduleMap.tenantName, industryPack: packId });
    setModuleMap(next);
    saveTenantModuleMap(next);
    const nextEnabled = [...next.enabledCanonicalPortals, ...next.enabledIndustryPortals];
    setEnabledPortals(nextEnabled);
    saveEnabledPortals(nextEnabled);
  };
  const reset = () => { const next = createTenantModuleMap({ tenantId: 'steelcraft-default', tenantName: 'Steel Craft', industryPack: 'metal_buildings' }); resetTenant(next); window.location.reload(); };
  const rows = [['Canonical core', 'Admin, Accounting, Contacts, HR, Vendor, Customer, Employee stay reusable for every tenant.', 'Core'], ['Selected industry pack', `${selectedPack.title}: ${selectedPack.description}`, 'Active'], ['Active industry portals', selectedPack.portals.map((p) => p.title).join(', '), selectedPack.id], ['Developer lock', locked ? 'Developer room is marked locked for handoff.' : 'Developer room is editable during prototype.', 'Local']];
  return <><header className="workspace-header panel"><div><p className="eyebrow">Developer Room</p><h1>Package installer</h1><p>Pick the industry pack that gets pushed down to this tenant. Non-canonical portals only become active through the selected industry package.</p></div><div className="live-badge">Industry packs</div></header><div className="workspace-grid"><article className="feature panel large"><p className="eyebrow">Industry pack selector</p><h2>Choose what this customer gets</h2><p>Canonical core remains stable. Sales, estimating, projects, planning, purchasing, production, service, and other non-core modules come from the selected industry pack.</p><label>Installed industry pack<select value={moduleMap.industryPack} disabled={locked} onChange={(event) => selectPack(event.target.value)}>{industryPackOptions.map((pack) => <option value={pack.id} key={pack.id}>{pack.title}</option>)}</select></label><div className="quote-actions"><button type="button" onClick={() => goTo('/brand')}>Open Brand Room</button><button type="button" onClick={reset}>Reset to Metal Buildings</button><button type="button" onClick={locked ? () => { unlockDeveloperRoom(); setLocked(false); } : () => { lockDeveloperRoom(); setLocked(true); }}>{locked ? 'Unlock Developer' : 'Lock Developer'}</button></div></article><RecordList title="Module map" rows={rows} /></div><article className="feature panel access-manager"><p className="eyebrow">Available industry packs</p><h2>Package catalog</h2><div className="portal-permission-grid">{industryPackOptions.map((pack) => <button type="button" className={`permission-toggle ${moduleMap.industryPack === pack.id ? 'active' : ''}`} disabled={locked} key={pack.id} onClick={() => selectPack(pack.id)}><span><strong>{pack.title}</strong><small>{pack.description}</small></span></button>)}</div></article><article className="feature panel access-manager"><p className="eyebrow">Canonical Core</p><h2>Reusable portals every tenant can share</h2><div className="portal-permission-grid">{canonicalRows.map(([id, title, kind]) => <label className="permission-toggle" key={id}><input type="checkbox" disabled={locked} checked={enabledPortals.includes(id)} onChange={() => togglePortal(id)} /><span><strong>{title}</strong><small>{kind} · core · {portalUrl(id)}</small></span></label>)}</div></article><article className="feature panel access-manager"><p className="eyebrow">Selected Industry Pack / {selectedPack.title}</p><h2>Modules pushed down to this tenant</h2><div className="portal-permission-grid">{industryRows.map(([id, title, kind, desc, record]) => <label className="permission-toggle" key={id}><input type="checkbox" disabled={locked} checked={enabledPortals.includes(id)} onChange={() => togglePortal(id)} /><span><strong>{title}</strong><small>{kind} · {record.package} · {portalUrl(id)}</small></span></label>)}</div></article></>;
}

function App() {
  const [moduleMap, setModuleMap] = useState(loadTenantModuleMap);
  const portalRecords = useMemo(() => getTenantPortals(moduleMap), [moduleMap]);
  const portals = useMemo(() => portalRecords.map(portalTuple), [portalRecords]);
  const customerPortalIds = useMemo(() => portalRecords.map((portal) => portal.id), [portalRecords]);
  const [enabledPortals, setEnabledPortals] = useState(() => loadEnabledPortals(loadTenantModuleMap()));
  ...
  const [user, setUser] = useState(() => loadSession(moduleMap));
  ...
  return <Shell brand={brand} user={user} active={active} signOut={signOut} enabledPortals={enabledPortals} portals={portals} meta={meta}>{active === 'admin' ? <Admin /> : active === 'accounting' ? <AccountingPortal /> : isLiveCanonicalPortal(active) ? <LiveCanonicalPortal id={active} Header={Header} /> : <Portal id={active} />}</Shell>;
}
