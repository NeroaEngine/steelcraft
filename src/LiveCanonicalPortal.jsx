...
function AccountDetail({ account, linkedContacts, accountTypes, industries, colorMap, optionMeta, close, patchAccount, patchContact, addOption, setValueColor, addContact, openContact }) {
...
function ContactsCrm({ Header, id }) {
  ...
  const [optionMeta, setOptionMeta] = useState(loadOptionMeta);
  ...
  useEffect(() => { localStorage.setItem(optionMetaKey, JSON.stringify(optionMeta)); }, [optionMeta]);
  ...
  function addOption(kind, value, meta = {}) { if (!value) return; setOptions((current) => ({ ...current, [kind]: uniq([...(current[kind] || []), value]) })); if (meta?.description || meta?.createdAt) setOptionMeta((current) => ({ ...current, [value]: { kind, description: meta.description || '', createdAt: meta.createdAt || new Date().toISOString() } })); if (meta?.color) setValueColor(value, meta.color); }
