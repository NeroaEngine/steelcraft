...
const colors = { 'GC/Developers/Owner': '#f6a637', 'General Contractors': '#4f8ff0', 'Architect / Engineering Firms': '#5bc3e6', 'Architect/Engineer': '#5bc3e6', Engineers: '#62c6ee', 'Material Suppliers / Fabricators / Vendors': '#0a7ca4', Suppliers: '#9a4ed6', Erectors: '#dd79b8', 'Testing & Inspection Agencies': '#5b8df5', 'Inspection & Testing': '#5755db', 'Insurance / Surety': '#c7aa2f', Financial: '#9bd717', 'Financial Institutions': '#ff5a35', 'Logistics / Freight': '#9656d8', Owner: '#f90078', 'Project Owner': '#06834b', "Sub-Contractor PM's": '#22508c', 'No Label': '#777' };
...
const styles = {
  ...
  top: { display: 'grid', gridTemplateColumns: 'minmax(360px,1fr) minmax(360px,520px)', gap: 16, alignItems: 'start' },
  ...
  toolbar: { display: 'grid', gridTemplateColumns: 'minmax(160px,230px) minmax(230px,330px) minmax(220px,1fr) auto', gap: 12, alignItems: 'end', margin: '14px 0' },
  tableWrap: { width: '100%', overflowX: 'auto', border: '1px solid var(--line)', borderRadius: 16, background: 'rgba(255,255,255,.025)' },
  table: { minWidth: 1740, display: 'grid' },
  accountRow: { display: 'grid', gridTemplateColumns: '270px 260px 330px 240px 320px 150px 92px', gap: 0, alignItems: 'stretch' },
  contactRow: { display: 'grid', gridTemplateColumns: '260px 230px 330px 190px 180px 310px 92px', gap: 0, alignItems: 'stretch' },
  ...
  cell: { minHeight: 58, padding: 9, borderRight: '1px solid var(--line)', borderBottom: '1px solid var(--line)', display: 'grid', alignItems: 'center' },
  pager: { display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'flex-end', flexWrap: 'wrap', padding: '12px 0 0' },
  pill: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, padding: '8px 10px', color: '#fff', fontWeight: 900, whiteSpace: 'nowrap', background: '#666' },
...
function SelectCell({ value, options, onChange, onAdd }) { return <select style={inputStyle} value={value || ''} onChange={(event) => { const value = event.target.value; if (value === '__add__') { const next = window.prompt('Add a new option'); if (next) onAdd(next.trim()); return; } onChange(value); }}><option value="">No value</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}<option value="__add__">+ Add new option</option></select>; }
function LinkCell({ value, onChange, kind }) { const href = kind === 'email' ? (emailFor(value) ? `mailto:${emailFor(value)}` : '') : hrefFor(value); return <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 6 }}><TextCell value={value} onChange={onChange} placeholder={kind === 'email' ? 'email' : 'domain or URL'} />{href ? <a style={styles.button} href={href} target={kind === 'email' ? undefined : '_blank'} rel="noreferrer">Open</a> : null}</div>; }
...
function ContactsCrm({ Header, id }) {
  ...
  return <>
    ...
    <section className="live-module-shell canonical-force-live" style={styles.shell}><article className="live-module-card" style={styles.card}>
      <div style={styles.top}><div><p className="eyebrow">Steel Craft CRM</p><h2 style={{ fontSize: 40, margin: '4px 0' }}>{tab === 'accounts' ? 'Accounts' : 'Contacts'}</h2><p style={{ maxWidth: 820, margin: 0 }}>Search, filter, edit cells, add rows, add dropdown options, open domains, and email contacts.</p></div><div style={{ display: 'grid', gap: 10 }}><div style={{ ...styles.tabs, justifyContent: 'flex-end' }}><button type="button" style={tab === 'accounts' ? styles.activeButton : styles.button} onClick={() => switchTab('accounts')}>Accounts</button><button type="button" style={tab === 'contacts' ? styles.activeButton : styles.button} onClick={() => switchTab('contacts')}>Contacts</button></div><input style={inputStyle} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search CRM" /></div></div>
      <div style={styles.toolbar}><div><span className="eyebrow">Showing</span><strong>{rangeStart}-{rangeEnd} of {filtered.length} filtered / {rows.length} loaded</strong></div><label><span>Type filter</span><select style={inputStyle} value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="All">All {tab === 'accounts' ? 'account' : 'contact'} types</option>{labels.map((label) => <option value={label} key={label}>{label}</option>)}</select></label><div style={{ color: 'var(--muted)' }}>{tab === 'accounts' ? '144 accounts loaded. Use page controls at the bottom to move through every record.' : '186 contacts loaded. Use page controls at the bottom to move through every record.'}</div><button type="button" style={styles.activeButton} onClick={tab === 'accounts' ? addAccount : () => addContact(null)}>+ {tab === 'accounts' ? 'Account' : 'Contact'}</button></div>
      <div style={styles.tableWrap}><div style={styles.table}>{tab === 'accounts' ? <><Head rowStyle={styles.accountRow} columns={['Account', 'Account Type', 'Domain', 'Industry', 'Linked Contacts', 'Item ID', 'Open']} />{pageRows.map((account) => <div style={styles.accountRow} key={account.id}><Cell><TextCell value={account.name} onChange={(value) => patchAccount(account.id, { name: value })} /></Cell><Cell><SelectCell value={account.type} options={accountTypes} onChange={(value) => patchAccount(account.id, { type: value })} onAdd={(value) => { addOption('accountTypes', value); patchAccount(account.id, { type: value }); }} /></Cell><Cell><LinkCell value={account.domain} onChange={(value) => patchAccount(account.id, { domain: value })} /></Cell><Cell><SelectCell value={account.industry} options={industries} onChange={(value) => patchAccount(account.id, { industry: value })} onAdd={(value) => { addOption('industries', value); patchAccount(account.id, { industry: value }); }} /></Cell><Cell><div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 6 }}><TextCell value={names(account.contacts)} onChange={(value) => patchAccount(account.id, { contacts: splitNames(value) })} /><button type="button" style={styles.button} onClick={() => addContact(account)}>+</button></div></Cell><Cell><span>{account.itemId}</span></Cell><Cell><button type="button" style={styles.button} onClick={() => setActive({ kind: 'account', id: account.id })}>Open</button></Cell></div>)}</> : <><Head rowStyle={styles.contactRow} columns={['Contact', 'Type', 'Linked Account', 'Title', 'Phone', 'Email', 'Open']} />{pageRows.map((contact) => <div style={styles.contactRow} key={contact.id}><Cell><TextCell value={contact.name} onChange={(value) => patchContact(contact.id, { name: value })} /></Cell><Cell><SelectCell value={contact.type} options={contactTypes} onChange={(value) => patchContact(contact.id, { type: value })} onAdd={(value) => { addOption('contactTypes', value); patchContact(contact.id, { type: value }); }} /></Cell><Cell><select style={inputStyle} value={contact.accountId || ''} onChange={(event) => { const account = crm.accounts.find((row) => row.id === event.target.value); patchContact(contact.id, { accountId: account?.id || '', linkedAccount: account?.name || '' }); }}><option value="">No linked account</option>{crm.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></Cell><Cell><TextCell value={contact.title} onChange={(value) => patchContact(contact.id, { title: value })} /></Cell><Cell><TextCell value={contact.phone} onChange={(value) => patchContact(contact.id, { phone: value })} /></Cell><Cell><LinkCell kind="email" value={contact.email} onChange={(value) => patchContact(contact.id, { email: value })} /></Cell><Cell><button type="button" style={styles.button} onClick={() => setActive({ kind: 'contact', id: contact.id })}>Open</button></Cell></div>)}</>}</div></div>
      <div style={styles.pager}><button type="button" style={styles.button} disabled={safePage <= 1} onClick={() => setPage(1)}>First</button><button type="button" style={styles.button} disabled={safePage <= 1} onClick={() => setPage((current) => clampPage(current - 1, totalPages))}>Prev</button><strong>Page {safePage} of {totalPages}</strong><button type="button" style={styles.button} disabled={safePage >= totalPages} onClick={() => setPage((current) => clampPage(current + 1, totalPages))}>Next</button><button type="button" style={styles.button} disabled={safePage >= totalPages} onClick={() => setPage(totalPages)}>Last</button><label style={{ margin: 0, minWidth: 130 }}><span>Rows/page</span><select style={inputStyle} value={pageSize} onChange={(event) => setPageSize(Number(event.target.value))}><option value={25}>25</option><option value={50}>50</option><option value={100}>100</option><option value={144}>144</option></select></label></div>
    </article></section>
    {activeAccount ? <RecordModal title={activeAccount.name} close={() => setActive(null)}><div style={{ display: 'grid', gap: 12 }}><LinkCell value={activeAccount.domain} onChange={(value) => patchAccount(activeAccount.id, { domain: value })} /><SelectCell value={activeAccount.type} options={accountTypes} onChange={(value) => patchAccount(activeAccount.id, { type: value })} onAdd={(value) => { addOption('accountTypes', value); patchAccount(activeAccount.id, { type: value }); }} /><button type="button" style={styles.activeButton} onClick={() => addContact(activeAccount)}>+ Add linked contact</button></div></RecordModal> : null}
