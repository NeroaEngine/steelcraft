...
const seedContacts = [
  { id: 'con-9902047458', name: 'Andy Wilkin', type: 'General Contractors', accountId: 'acc-9902047458', linkedAccount: 'Accu-Tech Construction, Inc.', title: '', phone: '', email: 'andy@accutech1.com', url: '', notes: '', itemId: '9902047458' },
  { id: 'con-justin', name: 'Justin R. Hammond', type: 'General Contractors', accountId: 'acc-10060908348', linkedAccount: 'Hammond Contracting, LLC.', title: '', phone: '', email: '', url: '', notes: '', itemId: 'con-justin' },
  { id: 'con-rob', name: 'Rob Blount', type: 'Architect/Engineer', accountId: 'acc-rbd', linkedAccount: 'RBD Design', title: '', phone: '', email: '', url: '', notes: '', itemId: 'con-rob' },
  ...Array.from({ length: 183 }, (_, index) => {
    const account = seedAccounts[index % seedAccounts.length];
    return { id: `con-preview-${index + 1}`, name: `Imported Contact ${index + 1}`, type: contactTypeSeeds[index % contactTypeSeeds.length], accountId: account.id, linkedAccount: account.name, title: '', phone: '', email: '', url: '', notes: '', itemId: `contact-preview-${index + 1}` };
  })
];
...
function normalizeCrm(data) { return { accounts: (data?.accounts?.length ? data.accounts : seedAccounts).map((account) => ({ accountEmail: '', gmailLink: '', neroaLink: '', notes: '', contacts: [], ...account })), contacts: (data?.contacts?.length ? data.contacts : seedContacts).map((contact) => ({ url: '', notes: '', ...contact })) }; }
