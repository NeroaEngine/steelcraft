...
const styles = {
  ...
  table: { minWidth: 2440, display: 'grid' },
  accountRow: { display: 'grid', gridTemplateColumns: '300px 360px 124px 520px 300px 124px 430px 160px 100px', gap: 0, alignItems: 'stretch' },
...
function ContactsCrm({ Header, id }) {
  ...
  return <>
    ...
    {activeAccount ? <RecordModal title={activeAccount.name} close={() => setActive(null)}><div style={{ display: 'grid', gap: 12 }}><LinkCell value={activeAccount.domain} onChange={(value) => patchAccount(activeAccount.id, { domain: value })} /><SelectCell value={activeAccount.type} options={accountTypes} color={colorMap[activeAccount.type]} onChange={(value) => patchAccount(activeAccount.id, { type: value })} onAdd={(value) => { addOption('accountTypes', value); patchAccount(activeAccount.id, { type: value }); }} /><ColorCell value={activeAccount.type} color={colorMap[activeAccount.type]} onColor={(color) => setValueColor(activeAccount.type, color)} /><button type="button" style={styles.activeButton} onClick={() => addContact(activeAccount)}>+ Add linked contact</button></div></RecordModal> : null}
    {activeContact ? <RecordModal title={activeContact.name} close={() => setActive(null)}><div style={{ display: 'grid', gap: 12 }}><SelectCell value={activeContact.type} options={contactTypes} color={colorMap[activeContact.type]} onChange={(value) => patchContact(activeContact.id, { type: value })} onAdd={(value) => { addOption('contactTypes', value); patchContact(activeContact.id, { type: value }); }} /><ColorCell value={activeContact.type} color={colorMap[activeContact.type]} onColor={(color) => setValueColor(activeContact.type, color)} /><TextCell value={activeContact.title} onChange={(value) => patchContact(activeContact.id, { title: value })} /><TextCell value={activeContact.phone} onChange={(value) => patchContact(activeContact.id, { phone: value })} /><LinkCell kind="email" value={activeContact.email} onChange={(value) => patchContact(activeContact.id, { email: value })} /></div></RecordModal> : null}
  </>;
}
