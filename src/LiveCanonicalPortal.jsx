...
].map(([id, name, type, domain, industry, contacts, itemId]) => ({ id, name, type, domain, industry, contacts, itemId, accountEmail: '', emailProviderLink: '', emailProviderName: '', neroaLink: '', notes: '', notesLog: [], emailLog: [], description: '', employees: '', headquarters: '', salesEstimating: '' }));
...
const seedContacts = [
  { id: 'con-9902047458', name: 'Andy Wilkin', type: 'General Contractors', accountId: 'acc-9902047458', linkedAccount: 'Accu-Tech Construction, Inc.', title: '', phone: '', email: 'andy@accutech1.com', url: '', notes: '', notesLog: [{ id: 'note-andy-1', date: '2026-01-05', person: 'Andy Wilkin', body: 'Email thread imported from Monday activity timeline for Tomoka Landfill PEMB RFI.', source: 'Manual note' }], emailLog: [{ id: 'email-andy-1', date: '2026-01-05', from: 'deb.moore@accutech1.com', subject: 'Re: Tomoka Landfill PEMB RFI', provider: 'Linked email provider', summary: 'Subcontractor bid documents and certifications were requested for Accu-Tech bid submittal.' }], itemId: '9902047458' },
  { id: 'con-justin', name: 'Justin R. Hammond', type: 'General Contractors', accountId: 'acc-10060908348', linkedAccount: 'Hammond Contracting, LLC.', title: '', phone: '', email: '', url: '', notes: '', notesLog: [], emailLog: [], itemId: 'con-justin' },
  { id: 'con-rob', name: 'Rob Blount', type: 'Architect/Engineer', accountId: 'acc-rbd', linkedAccount: 'RBD Design', title: '', phone: '', email: '', url: '', notes: '', notesLog: [], emailLog: [], itemId: 'con-rob' },
  ...Array.from({ length: 183 }, (_, index) => {
    const account = seedAccounts[index % seedAccounts.length];
    return { id: `con-preview-${index + 1}`, name: `Imported Contact ${index + 1}`, type: contactTypeSeeds[index % contactTypeSeeds.length], accountId: account.id, linkedAccount: account.name, title: '', phone: '', email: '', url: '', notes: '', notesLog: [], emailLog: [], itemId: `contact-preview-${index + 1}` };
  })
];

const crmKey = 'steelcraft_crm_records_v7';
const oldCrmKeys = ['steelcraft_crm_records_v6', 'steelcraft_crm_records_v5', 'steelcraft_live_crm_records_v3'];
...
function normalizeAccount(account) { return { accountEmail: '', emailProviderLink: account.gmailLink || '', emailProviderName: '', neroaLink: '', notes: '', notesLog: [], emailLog: [], contacts: [], ...account }; }
function normalizeContact(contact) { return { url: '', notes: '', notesLog: [], emailLog: [], ...contact }; }
  ...
  return <ModalShell title={account.name} subtitle="Account workspace with daily notes, linked email provider, Neroa Connect, and linked contacts." close={close}>
    <div style={styles.modalGrid}>
      <div style={{ display: 'grid', gap: 14 }}>
        ...
        <section style={styles.panel}><h3 style={{ margin: 0 }}>Links</h3><label><span>Website / domain</span><LinkCell value={account.domain || ''} onChange={(value) => patchAccount(account.id, { domain: value })} /></label><label><span>Link Email Provider</span><LinkCell value={account.emailProviderLink || ''} label="Link Email Provider" onChange={(value) => patchAccount(account.id, { emailProviderLink: value })} /></label><label><span>Email provider name</span><TextCell value={account.emailProviderName || ''} onChange={(value) => patchAccount(account.id, { emailProviderName: value })} placeholder="Outlook, Microsoft 365, Gmail, Yahoo..." /></label><label><span>Neroa Connect link / reference</span><LinkCell value={account.neroaLink || ''} onChange={(value) => patchAccount(account.id, { neroaLink: value })} /></label><div style={{ ...styles.tabs, marginTop: 4 }}><button type="button" style={styles.activeButton} onClick={() => { if (!accountEmail && !account.emailProviderLink) { const next = window.prompt('Add an account email or provider link'); if (next) patchAccount(account.id, { accountEmail: next }); return; } openExternal(providerTarget); }}>Open Email Provider</button><button type="button" style={styles.activeButton} onClick={() => handoffToNeroa('account', account)}>Send to Neroa Connect</button></div></section>
...
function ContactsCrm({ Header, id }) {
  ...
  function addAccount() { const id = `acc-new-${Date.now()}`; setCrm((current) => ({ ...current, accounts: [{ id, name: 'New Account', type: 'General Contractors', domain: '', industry: '', contacts: [], accountEmail: '', emailProviderLink: '', emailProviderName: '', neroaLink: '', notes: '', notesLog: [], emailLog: [], itemId: `new-${current.accounts.length + 1}` }, ...current.accounts] })); setTab('accounts'); setPage(1); setActive({ kind: 'account', id }); }
  function addContact(account) { const id = `con-new-${Date.now()}`; const next = { id, name: 'New Contact', type: 'General Contractors', accountId: account?.id || '', linkedAccount: account?.name || '', title: '', phone: '', email: '', url: '', notes: '', notesLog: [], emailLog: [], itemId: `new-contact-${crm.contacts.length + 1}` }; setCrm((current) => ({ ...current, contacts: [next, ...current.contacts], accounts: account ? current.accounts.map((row) => row.id === account.id ? { ...row, contacts: uniq([...(row.contacts || []), next.name]) } : row) : current.accounts })); setTab('contacts'); setPage(1); setActive({ kind: 'contact', id }); }
