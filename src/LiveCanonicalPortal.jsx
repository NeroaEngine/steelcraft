...
const emailProviderApps = [
  { id: 'google-workspace', name: 'Google Workspace', auth: 'OAuth', description: 'Use the Google mail integration app when it is installed.' },
  { id: 'microsoft-365', name: 'Microsoft 365 / Outlook', auth: 'OAuth', description: 'Use the Microsoft Graph / Outlook integration app when it is installed.' },
  { id: 'yahoo-mail', name: 'Yahoo Mail', auth: 'OAuth or app password', description: 'Use Yahoo provider setup or app-password based mail access.' },
  { id: 'icloud-mail', name: 'iCloud Mail', auth: 'App password', description: 'Use iCloud app-password based mail access.' },
  { id: 'imap-smtp', name: 'IMAP / SMTP', auth: 'Mailbox settings', description: 'Use generic internet mail settings for providers that support IMAP.' },
  { id: 'custom-provider', name: 'Custom Email Provider', auth: 'Custom', description: 'Use this for any other internet email provider or future integration app.' }
];
...
const crmKey = 'steelcraft_crm_records_v8';
const oldCrmKeys = ['steelcraft_crm_records_v7', 'steelcraft_crm_records_v6', 'steelcraft_crm_records_v5', 'steelcraft_live_crm_records_v3'];
...
const styles = {
  ...
  providerCard: { border: '1px solid var(--line)', borderRadius: 14, background: 'rgba(0,0,0,.2)', padding: 12, display: 'grid', gap: 8, textAlign: 'left' },
...
function saveProviderContext(record, provider) { const payload = { source: 'steelcraft-crm', action: 'email-provider-setup', recordId: record.id, recordName: record.name, providerId: provider.id, providerName: provider.name, timestamp: new Date().toISOString() }; localStorage.setItem('steelcraft_email_provider_setup_context', JSON.stringify(payload)); window.dispatchEvent(new CustomEvent('steelcraft-email-provider-setup', { detail: payload })); }
...
function EmailProviderSetup({ record, onSave }) {
  const [open, setOpen] = useState(false);
  const [customName, setCustomName] = useState(record.emailProviderName || '');
  const [customLink, setCustomLink] = useState(record.emailProviderLink || '');
  const selected = record.emailProviderName || 'No provider linked';
  function chooseProvider(provider) {
    onSave({ emailProviderName: provider.name, emailProviderId: provider.id, emailProviderStatus: 'Selected - ready for integration app setup' });
    saveProviderContext(record, provider);
    setOpen(false);
  }
  function saveCustom() {
    const provider = { id: 'custom-provider', name: customName || 'Custom Email Provider' };
    onSave({ emailProviderName: provider.name, emailProviderId: provider.id, emailProviderLink: customLink, emailProviderStatus: 'Custom provider saved - ready for setup' });
    saveProviderContext(record, provider);
    setOpen(false);
  }
  return <div style={{ display: 'grid', gap: 10 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}><div><strong>Link Email Provider</strong><div style={{ color: 'var(--muted)' }}>{selected}</div>{record.emailProviderStatus ? <span style={styles.statusPill}>{record.emailProviderStatus}</span> : null}</div><button type="button" style={styles.activeButton} onClick={() => setOpen((value) => !value)}>Link Email Provider</button></div>{open ? <div style={{ display: 'grid', gap: 12 }}><div style={styles.providerGrid}>{emailProviderApps.map((provider) => <button type="button" key={provider.id} style={styles.providerCard} onClick={() => chooseProvider(provider)}><strong>{provider.name}</strong><span style={styles.statusPill}>{provider.auth}</span><small style={{ color: 'var(--muted)', lineHeight: 1.35 }}>{provider.description}</small></button>)}</div><div style={styles.panel}><h4 style={{ margin: 0 }}>Custom provider / integration app</h4><div style={styles.detailGrid}><label><span>Provider name</span><TextCell value={customName} onChange={setCustomName} placeholder="Any internet email provider" /></label><label><span>Provider setup URL / app route</span><TextCell value={customLink} onChange={setCustomLink} placeholder="Paste integration app URL or setup route" /></label></div><button type="button" style={styles.activeButton} onClick={saveCustom}>Save Custom Provider</button></div></div> : null}</div>;
}
...
function AccountDetail({ account, linkedContacts, accountTypes, industries, colorMap, close, patchAccount, patchContact, addOption, setValueColor, addContact, openContact }) {
  ...
  return <ModalShell title={account.name} subtitle="Account workspace with daily notes, linked email provider, Neroa Connect, and linked contacts." close={close}>
    <div style={styles.modalGrid}>
      <div style={{ display: 'grid', gap: 14 }}>
        ...
        <section style={styles.panel}><h3 style={{ margin: 0 }}>Links</h3><label><span>Website / domain</span><LinkCell value={account.domain || ''} onChange={(value) => patchAccount(account.id, { domain: value })} /></label><EmailProviderSetup record={account} onSave={(patch) => patchAccount(account.id, patch)} /><label><span>Provider setup URL / app route</span><LinkCell value={account.emailProviderLink || ''} label="Edit Provider Link" onChange={(value) => patchAccount(account.id, { emailProviderLink: value })} /></label><label><span>Neroa Connect link / reference</span><LinkCell value={account.neroaLink || ''} onChange={(value) => patchAccount(account.id, { neroaLink: value })} /></label><div style={{ ...styles.tabs, marginTop: 4 }}><button type="button" style={styles.activeButton} onClick={() => { if (!account.emailProviderName && !account.emailProviderLink) return window.alert('Choose Link Email Provider first.'); if (providerTarget) openExternal(providerTarget); }}>Open Email Provider</button><button type="button" style={styles.activeButton} onClick={() => handoffToNeroa('account', account)}>Send to Neroa Connect</button></div></section>
        <ActivityTimeline record={account} linkedEmails={linkedEmails} onAddNote={(note) => patchAccount(account.id, { notesLog: [note, ...(account.notesLog || [])] })} />
        <section style={styles.panel}><h3 style={{ margin: 0 }}>Legacy notes</h3><TextAreaCell value={account.notes || ''} placeholder="Long-form notes can stay here if needed." onChange={(value) => patchAccount(account.id, { notes: value })} /></section>
      </div>
      <aside style={styles.panel}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}><h3 style={{ margin: 0 }}>Linked contacts</h3><button type="button" style={styles.activeButton} onClick={() => addContact(account)}>+ Contact</button></div><TextCell value={names(account.contacts)} onChange={(value) => patchAccount(account.id, { contacts: splitNames(value) })} placeholder="Linked contact names" />{linkedContacts.length ? linkedContacts.map((contact) => <div style={styles.linkedRow} key={contact.id}><div><strong>{contact.name}</strong><small style={{ display: 'block', color: 'var(--muted)' }}>{contact.email || contact.type || 'No email loaded'}</small></div><button type="button" style={styles.button} onClick={() => openContact(contact.id)}>Open</button><button type="button" style={styles.button} onClick={() => { if (contact.email) openExternal(emailProviderUrl(contact.email, `Steel Craft CRM - ${account.name}`)); else { const next = window.prompt(`Add email for ${contact.name}`); if (next) patchContact(contact.id, { email: next }); } }}>Email</button></div>) : <p style={{ color: 'var(--muted)' }}>No linked contact records yet. Add names above or create a contact.</p>}</aside>
    </div>
  </ModalShell>;
}
...
function ContactsCrm({ Header, id }) {
  ...
  return <>
    ...
    {activeContact ? <ContactDetail contact={activeContact} account={activeContactAccount} accounts={crm.accounts} contactTypes={contactTypes} colorMap={colorMap} close={() => setActive(null)} patchContact={patchContact} addOption={addOption} setValueColor={setValueColor} /> : null}
  </>;
}
