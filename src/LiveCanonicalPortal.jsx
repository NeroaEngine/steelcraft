...
const accountTypeLabels = ['All account types', 'GC/Developers/Owner', 'General Contractors', 'Architect / Engineering Firms', 'Engineers', 'Material Suppliers / Fabricators / Vendors', 'Erectors', 'Testing & Inspection Agencies', 'Insurance / Surety', 'Financial Institutions', 'Logistics / Freight', 'Owner'];
const contactTypeLabels = ['All contact types', 'General Contractors', 'Financial', 'Project Owner', 'Suppliers', 'Architect/Engineer', 'No Label', 'Erectors', "Sub-Contractor PM's", 'Inspection & Testing'];
...
const generatedAccounts = Array.from({ length: 45 }, (_, index) => {
  const type = accountTypeLabels[(index % (accountTypeLabels.length - 1)) + 1];
  return { id: `acc-preview-${index + 1}`, name: `Imported Monday Account ${index + 1}`, type, domain: '-', industry: index % 2 ? 'Construction' : 'Vendor / Services', description: 'Placeholder row showing the wide board scroll behavior while the remaining Monday export rows are loaded into the app dataset.', employees: '', headquarters: '', contacts: [`Imported Contact ${index + 1}`], salesEstimating: '', itemId: `preview-${index + 1}` };
});

const accounts = [...coreAccounts, ...generatedAccounts];
...
const contacts = [
  ...
  ...Array.from({ length: 65 }, (_, index) => ({ id: `con-preview-${index + 1}`, name: `Imported Contact ${index + 1}`, firstName: 'Imported', lastName: `Contact ${index + 1}`, type: contactTypeLabels[(index % (contactTypeLabels.length - 1)) + 1], accountId: generatedAccounts[index % generatedAccounts.length].id, linkedAccount: generatedAccounts[index % generatedAccounts.length].name, title: '', phone: '', email: '', comments: '', company: '', salesEstimating: '', itemId: `contact-preview-${index + 1}`, updates: [{ kind: 'Record', date: 'Imported from Monday export', from: 'Monday CRM', to: 'Steel Craft CRM', subject: 'Contact imported', body: `Imported Contact ${index + 1} is linked to ${generatedAccounts[index % generatedAccounts.length].name}.`, attachments: [] }] }))
];

const colors = { 'GC/Developers/Owner': '#f6a637', 'GC/Developers/Owners': '#f6a637', 'General Contractors': '#4f8ff0', 'Architect / Engineering Firms': '#5bc3e6', 'Architect/Engineer': '#5bc3e6', Engineers: '#62c6ee', 'Material Suppliers / Fabricators / Vendors': '#0a7ca4', Suppliers: '#9a4ed6', Erectors: '#dd79b8', 'Testing & Inspection Agencies': '#5b8df5', 'Inspection & Testing': '#5755db', 'Insurance / Surety': '#c7aa2f', Financial: '#9bd717', 'Financial Institutions': '#ff5a35', 'Logistics / Freight': '#9656d8', Owner: '#f90078', 'Project Owner': '#06834b', "Sub-Contractor PM's": '#22508c', 'No Label': '#bbb' };

const styles = {
  shell: { width: 'calc(100vw - 56px)', maxWidth: 'none', margin: '0 auto', display: 'grid', gap: 18 },
  card: { width: '100%', maxWidth: 'none', border: '1px solid var(--line)', borderRadius: 24, background: 'var(--card)', padding: 24, boxShadow: '0 18px 48px rgba(0,0,0,.2)' },
  top: { display: 'grid', gridTemplateColumns: 'minmax(420px, 1fr) minmax(360px, 520px)', gap: 18, alignItems: 'start' },
  ...
  search: { minWidth: 320, width: '100%' },
  filterBar: { display: 'grid', gridTemplateColumns: '220px minmax(240px, 360px) 1fr', gap: 12, alignItems: 'end', margin: '18px 0' },
  tableWrap: { maxHeight: '62vh', overflowY: 'auto', overflowX: 'auto', paddingRight: 4, borderTop: '1px solid var(--line)' },
  table: { display: 'grid', gap: 8, minWidth: '1500px', paddingTop: 12 },
  accountRow: { display: 'grid', gridTemplateColumns: '1.45fr 1fr 1.25fr 1fr 1.35fr .9fr .45fr', gap: 14, alignItems: 'center' },
  contactRow: { display: 'grid', gridTemplateColumns: '1.1fr .9fr 1.35fr .8fr .8fr 1.15fr .45fr', gap: 14, alignItems: 'center' },
  ...
  modal: { width: 'min(1700px, calc(100vw - 44px))', borderRadius: 24, border: '1px solid var(--line)', background: 'var(--card)', color: 'var(--text)', boxShadow: '0 30px 90px rgba(0,0,0,.55)', overflow: 'hidden' },
  ...
  modalBody: { display: 'grid', gridTemplateColumns: 'minmax(620px, 1fr) 460px', gap: 0, minHeight: 620 },
...
function ContactsCrm({ Header, id }) {
  ...
  const labels = tab === 'accounts' ? accountTypeLabels : contactTypeLabels;
  const totalSourceCount = tab === 'accounts' ? 144 : 186;
  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    return data.filter((row) => {
      const selected = typeFilter.startsWith('All') || row.type === typeFilter;
  ...
  function switchTab(next) { setTab(next); setTypeFilter(next === 'accounts' ? 'All account types' : 'All contact types'); setQuery(''); }

  return <>
    {Header ? <Header id={id} /> : null}
    <section className="live-module-shell canonical-force-live" style={styles.shell}><article className="live-module-card" style={styles.card}><div style={styles.top}><div><p className="eyebrow">Steel Craft CRM</p><h2 style={{ fontSize: 46, margin: '6px 0' }}>{tab === 'accounts' ? 'Accounts Board' : 'Contacts Board'}</h2><p>Full-width Monday-style board replacement. Accounts are companies, contacts are people, and each contact opens into the activity/detail layout.</p></div><div style={{ display: 'grid', gap: 12, justifyItems: 'stretch' }}><div style={{ ...styles.tabs, justifyContent: 'flex-end' }}><button type="button" style={tab === 'accounts' ? styles.activeButton : styles.button} onClick={() => switchTab('accounts')}>Accounts</button><button type="button" style={tab === 'contacts' ? styles.activeButton : styles.button} onClick={() => switchTab('contacts')}>Contacts</button></div><input style={styles.search} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search CRM board..." /></div></div><div style={styles.filterBar}><div><span className="eyebrow">Showing</span><strong>{filtered.length} visible / {data.length} loaded / {totalSourceCount} source</strong></div><label><span>Type filter</span><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}>{labels.map((label) => <option key={label} value={label}>{label}</option>)}</select></label><div style={{ color: 'var(--muted)' }}>Labels now live inside the dropdown, not exposed across the top of the board.</div></div>{tab === 'accounts' ? <div style={styles.tableWrap}><div style={styles.table}><div style={{ ...styles.accountRow, ...styles.head }}><span>Account</span><span>Account Type</span><span>Domain</span><span>Industry</span><span>Linked Contacts</span><span>Item ID</span><span></span></div>{filtered.map((account) => <button type="button" key={account.id} style={styles.dataRow} onClick={() => setActiveAccount(account)}><div style={styles.accountRow}><strong>{account.name}</strong><Label value={account.type} /><span>{account.domain || '-'}</span><span>{account.industry || '-'}</span><span>{account.contacts.join(', ') || '-'}</span><span>{account.itemId}</span><b>Open</b></div></button>)}</div></div> : <div style={styles.tableWrap}><div style={styles.table}><div style={{ ...styles.contactRow, ...styles.head }}><span>Contact</span><span>Type</span><span>Linked Account</span><span>Title</span><span>Phone</span><span>Email</span><span></span></div>{filtered.map((contact) => <button type="button" key={contact.id} style={styles.dataRow} onClick={() => setActiveContact(contact)}><div style={styles.contactRow}><strong>{contact.name}</strong><Label value={contact.type} /><span>{contact.linkedAccount}</span><span>{contact.title || '-'}</span><span>{contact.phone || '-'}</span><span>{contact.email || '-'}</span><b>Open</b></div></button>)}</div></div>}</article></section>{activeAccount && <AccountModal account={activeAccount} close={() => setActiveAccount(null)} />}{activeContact && <ContactModal contact={activeContact} close={() => setActiveContact(null)} />}</>;
}
