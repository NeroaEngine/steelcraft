...
export default function CrmSalesWorkArea({ runtime }) {
  ...
  async function saveLeadActivity() {
    const event = await publishActivity({
      ...
      title: `CRM lead updated for ${lead.customer}`,
      body: `${lead.quoteStatus} · Estimator: ${lead.estimator} · Bid due: ${lead.bidDueDate}`,
  ...
  return (
    <section className="card crm-sales-workarea">
      <div className="section-heading">
        ...
        <h2>Lead Intake Through Contract Award</h2>
        <p>Capture the lead, customer, bid due date, estimator, proposal history, and follow-up schedule before the project moves into estimating.</p>
      </div>
      ...
      <div className="module-grid four-up">
        ...
        <article className="module">
          <Badge>Project Record</Badge>
          <h3>{runtime.project.projectNumber}</h3>
          <p>CRM activity attaches to the same Master Project Record used by estimating, billing, delivery, and erection.</p>
        </article>
        <article className="module">
          <Badge>Proposal History</Badge>
          <h3>Ready</h3>
          <p>Proposal versions and sent history will remain attached to this CRM opportunity.</p>
        </article>
        <article className="module">
          <Badge>Emails</Badge>
          <h3>Templates</h3>
      ...
      <div className="card">
        <div className="section-heading">
          ...
          <h2>CRM Activity</h2>
          <p>Every milestone creates an activity entry tied to the project record.</p>
        </div>
