(function () {
  ...
  function reportSupports(title, desc, category, basis, account) {
    var text = (title + ' ' + desc + ' ' + category).toLowerCase();
    var isCore = /general ledger|trial balance|balance sheet|profit and loss|executive|monthly close|audit|vault|evidence|exception/.test(text);
    var cashLike = /cash|bank|deposit|payment|reconciliation|unmatched|refund/.test(text);
    var accrualLike = /receivable|ar |customer|invoice|collections|payable|ap |vendor|bill|purchase order|po |sov|schedule of values|retainage|change order|project|job cost|committed|unbilled|underbilling|overbilling|revenue|expense|gross margin|tax|1099|insurance|lien/.test(text);
    if (basis === 'cash' && !(cashLike || isCore)) return false;
    if (basis === 'accrual' && !(accrualLike || isCore)) return false;
    if (!account || account === 'all') return true;
    if (account === 'cash') return cashLike || isCore;
    if (account === 'accrual') return accrualLike || isCore;
    var accountMap = {
      '1000': /cash|bank|deposit|payment|reconciliation|unmatched|balance sheet|general ledger|trial balance|cash position|profit and loss|executive/i,
      '1100': /receivable|ar |customer|invoice|collections|statement|credit memo|refund|balance sheet|general ledger|trial balance/i,
      '1200': /wip|unbilled|underbilling|overbilling|schedule of values|project|job cost|revenue|balance sheet|general ledger|trial balance/i,
      '2000': /payable|ap |vendor|bill|payment run|purchase order|po |expense|balance sheet|general ledger|trial balance/i,
      '2100': /retainage|lien|vendor|payable|project|balance sheet|general ledger|trial balance/i,
      '4000': /revenue|profit and loss|gross margin|forecast|invoice|schedule of values|general ledger|trial balance/i,
      '5000': /cost|cogs|job cost|purchase order|po |project|profitability|gross margin|profit and loss|general ledger|trial balance/i,
      '6000': /expense|bank|vendor|profit and loss|forecast|cash position|general ledger|trial balance/i
    };
    return accountMap[account] ? accountMap[account].test(text) : true;
  }
  function installStyle() {
    if (document.getElementById('accounting-reports-stable-style')) return;
    var s = document.createElement('style');
    s.id = 'accounting-reports-stable-style';
    s.textContent = '.accounting-report-room{display:block!important;width:min(100%,1180px)!important;max-width:1180px!important;margin:0 auto!important}.accounting-report-room>.accounting-form-card{display:none!important}.acct-report-shell,.acct-report-runner{display:grid!important;gap:20px!important;width:100%!important;max-width:1180px!important;box-sizing:border-box!important;margin:0 auto 24px!important;padding:28px!important;border:1px solid color-mix(in srgb,var(--line,#343036) 75%,var(--brand-accent,#9f3d42) 25%)!important;border-radius:var(--radius,22px)!important;background:color-mix(in srgb,var(--surface,#141418) 94%,#000 6%)!important}.acct-report-head h2,.acct-report-runner h1{margin:0!important;font-size:clamp(34px,3vw,48px)!important;line-height:1!important;letter-spacing:-.06em!important}.acct-report-head p,.acct-report-runner p{max-width:760px!important;line-height:1.45!important;color:var(--muted,#b7aaa3)!important}.acct-filter-status{display:flex!important;flex-wrap:wrap!important;gap:8px!important}.acct-filter-status span,.acct-status span,.acct-note{border:1px solid color-mix(in srgb,var(--line,#343036) 76%,transparent)!important;border-radius:999px!important;padding:9px 12px!important;color:var(--muted,#b7aaa3)!important;background:color-mix(in srgb,var(--surface-alt,#1e1e24) 62%,transparent)!important}.acct-report-filters,.acct-run-params{display:grid!important;grid-template-columns:repeat(6,minmax(0,1fr))!important;gap:14px!important;align-items:end!important}.acct-report-filters label,.acct-run-params label{display:grid!important;gap:7px!important;margin:0!important;min-width:0!important}.acct-report-filters span,.acct-run-params span{font-size:.72rem!important;font-weight:900!important;text-transform:uppercase!important;letter-spacing:.08em!important;color:color-mix(in srgb,var(--brand-accent,#9f3d42) 84%,white 16%)!important}.acct-report-filters input,.acct-report-filters select,.acct-run-params input,.acct-run-params select{width:100%!important;min-height:44px!important;border-radius:16px!important;box-sizing:border-box!important;padding:10px 12px!important}.acct-report-filters input[type=date],.acct-run-params input[type=date]{cursor:pointer!important;position:relative!important}.acct-report-groups{display:grid!important;gap:18px!important}.acct-report-group{display:grid!important;gap:12px!important}.acct-report-group-head{display:flex!important;justify-content:space-between!important;align-items:center!important;border-bottom:1px solid color-mix(in srgb,var(--line,#343036) 70%,transparent)!important;padding-bottom:8px!important}.acct-report-group-head h3{margin:0!important;font-size:1.05rem!important}.acct-report-card-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:12px!important}.acct-report-card{display:grid!important;grid-template-columns:minmax(180px,.8fr) minmax(260px,1.5fr) auto!important;gap:14px!important;align-items:center!important;width:100%!important;min-height:66px!important;text-align:left!important;border:1px solid color-mix(in srgb,var(--line,#343036) 78%,transparent)!important;border-radius:16px!important;background:color-mix(in srgb,var(--surface-alt,#1e1e24) 62%,transparent)!important;color:var(--text,#f6f0ea)!important;padding:14px 16px!important;cursor:pointer!important}.acct-report-card:hover{border-color:color-mix(in srgb,var(--brand-accent,#9f3d42) 54%,var(--line,#343036))!important}.acct-report-card span{color:var(--muted,#b7aaa3)!important;line-height:1.35!important}.acct-report-card b{justify-self:end!important;color:color-mix(in srgb,var(--brand-accent,#9f3d42) 86%,white 14%)!important}.acct-actions,.acct-status{display:flex!important;flex-wrap:wrap!important;gap:10px!important}.acct-actions button,.acct-back{border:1px solid color-mix(in srgb,var(--line,#343036) 82%,var(--brand-accent,#9f3d42) 18%)!important;background:color-mix(in srgb,var(--surface-alt,#1e1e24) 78%,#000 22%)!important;color:var(--text,#f6f0ea)!important;border-radius:14px!important;min-height:38px!important;padding:9px 13px!important;font-weight:900!important;cursor:pointer!important}.acct-actions [data-action=run]{background:color-mix(in srgb,var(--brand-accent,#9f3d42) 72%,#000 28%)!important}.acct-note{border-radius:16px!important}.acct-output{display:grid!important;gap:10px!important}.acct-result-row small{color:color-mix(in srgb,var(--brand-accent,#9f3d42) 80%,white 20%)!important;font-weight:800!important}@media(max-width:1180px){.acct-report-filters,.acct-run-params{grid-template-columns:repeat(3,minmax(0,1fr))!important}.acct-report-card-grid{grid-template-columns:1fr!important}}@media(max-width:720px){.acct-report-filters,.acct-run-params,.acct-report-card{grid-template-columns:1fr!important}.acct-report-card b{justify-self:start!important}.acct-report-shell,.acct-report-runner{padding:20px!important}}';
  ...
  function run(panel, action) {
    var title = panel.dataset.title, basis = panel.querySelector('[data-param=basis]').value, account = panel.querySelector('[data-param=account]').value;
    var accountText = panel.querySelector('[data-param=account] option:checked').textContent, from = panel.querySelector('[data-param=from]').value || 'period start', to = panel.querySelector('[data-param=to]').value || today();
  ...
  function openDatePicker(input) { if (!input || input.type !== 'date') return; input.focus({ preventScroll: true }); try { if (typeof input.showPicker === 'function') input.showPicker(); } catch (error) {} }
  function mount() { if (!onReportsPage()) return; installStyle(); var room = document.querySelector('.accounting-report-room'); if (!room || room.querySelector('[data-report-shell=true]')) return; var reports = getReports(); if (!reports.length) return; room.insertAdjacentHTML('afterbegin', shellHtml(reports)); filter(room.querySelector('[data-report-shell=true]')); }
  function filter(shell) {
    ...
    var account = shell.querySelector('[data-filter=account]').value;
    var basis = shell.querySelector('[data-filter=basis]').value;
    var from = shell.querySelector('[data-filter=from]').value;
    var to = shell.querySelector('[data-filter=to]').value;
    var total = 0;
    shell.querySelectorAll('[data-card=true]').forEach(function (card) {
      var text = (card.dataset.title + ' ' + card.dataset.desc).toLowerCase();
      var visible = (!search || text.indexOf(search) !== -1) && (category === 'all' || card.dataset.category === category) && reportSupports(card.dataset.title, card.dataset.desc, card.dataset.category, basis, account);
      card.hidden = !visible;
      if (visible) total += 1;
    });
    shell.querySelectorAll('[data-group]').forEach(function (group) {
      var count = Array.from(group.querySelectorAll('[data-card=true]')).filter(function (card) { return !card.hidden; }).length;
      group.hidden = count === 0;
      var label = group.querySelector('[data-group-count]');
      if (label) label.textContent = count + (count === 1 ? ' report' : ' reports');
    });
    var accountText = shell.querySelector('[data-filter=account] option:checked').textContent;
    var categoryText = shell.querySelector('[data-filter=category] option:checked').textContent;
    shell.querySelector('[data-filter-status]').innerHTML = '<span>Showing: ' + total + ' reports</span><span>Type: ' + esc(categoryText) + '</span><span>Account: ' + esc(accountText) + '</span><span>Basis: ' + esc(basis) + '</span>' + (from ? '<span>From: ' + esc(from) + '</span>' : '') + (to ? '<span>To: ' + esc(to) + '</span>' : '');
  }
  ...
  document.addEventListener('pointerdown', function (e) { var dateInput = e.target.closest('input[type="date"]'); if (!dateInput) { var dateLabel = e.target.closest('label'); if (dateLabel) dateInput = dateLabel.querySelector('input[type="date"]'); } if (dateInput) setTimeout(function () { openDatePicker(dateInput); }, 0); });
  document.addEventListener('click', function (e) {
    var dateInput = e.target.closest('input[type="date"]'); if (dateInput) { openDatePicker(dateInput); return; }
    var dateLabel = e.target.closest('label'); if (dateLabel) { var labelDateInput = dateLabel.querySelector('input[type="date"]'); if (labelDateInput) { openDatePicker(labelDateInput); return; } }
