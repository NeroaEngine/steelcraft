(function () {
  ...
  document.addEventListener('pointerdown', function (e) {
    ...
    if (!dateInput) {
      var dateLabel = e.target.closest('label');
      if (dateLabel) dateInput = dateLabel.querySelector('input[type="date"]');
    }
    if (dateInput) setTimeout(function () { openDatePicker(dateInput); }, 0);
  });
  document.addEventListener('click', function (e) {
    var dateInput = e.target.closest('input[type="date"]');
    if (dateInput) { openDatePicker(dateInput); return; }
    var dateLabel = e.target.closest('label');
    if (dateLabel) { var labelDateInput = dateLabel.querySelector('input[type="date"]'); if (labelDateInput) { openDatePicker(labelDateInput); return; } }
